...
3. 한 단계 `<이름>.opencodexpages.me` 주소를 고릅니다. 중앙 worker가 격리된 Cloudflare Tunnel을 만들고 GUI가 `pending`부터 `online`까지 상태를 추적합니다.
4. 서명된 로컬 헬퍼를 페어링합니다. 이후 주소에 접속할 때는 소유 GitHub 계정과 Remote 비밀번호를 모두 확인한 뒤 hostname 전용 세션을 만듭니다.

현재는 초대형 Linux MVP입니다. 관리자 권한이 필요한 서명된 헬퍼 installer와 Windows/macOS 헬퍼는 npm 패키지에 아직 포함되지 않습니다. GUI는 이 제한을 숨기지 않습니다. **Super Sync는 이 흐름의 범위가 아닙니다.**
...
| **Remote** | 이 PC를 중앙 GitHub 신원 서비스에 연결하고, 별도 접속 비밀번호와 hostname을 설정하고, Tunnel/Agent 상태를 확인합니다. |
...
| `POST /api/remote/activate` · `POST /api/remote/pairing-code` · `DELETE /api/remote/device` | hostname을 예약하고 일회용 Agent 페어링을 준비하거나 이 PC 연결을 해제합니다. |
