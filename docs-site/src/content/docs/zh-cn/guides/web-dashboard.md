...
3. 选择一级 `<名称>.opencodexpages.me` 地址；中央 worker 创建隔离的 Cloudflare Tunnel，GUI 跟踪从 `pending` 到 `online` 的状态。
4. 配对已签名的本地助手。以后访问该地址时，必须同时验证所属 GitHub 账号和 Remote 密码，Gateway 才会创建仅限该 hostname 的 session。

目前这是仅限邀请的 Linux MVP。需要管理员权限的签名助手 installer 及 Windows/macOS 助手尚未随 npm 包提供；GUI 会如实显示这一限制。**Super Sync 不属于此流程。**
...
| **Remote** | 将此电脑连接到中央 GitHub 身份服务，设置独立访问密码和 hostname，并跟踪 Tunnel/Agent 状态。 |
...
| `POST /api/remote/activate` · `POST /api/remote/pairing-code` · `DELETE /api/remote/device` | 保留 hostname、准备一次性 Agent 配对或撤销此电脑。 |
