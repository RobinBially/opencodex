...
| Remote | `GET /api/remote/status`, `POST /api/remote/link`, `PUT /api/remote/password`, `POST /api/remote/activate`, `POST /api/remote/pairing-code`, and `DELETE /api/remote/device` own the local-PC-first Remote flow. The browser DTO never includes the device polling secret, device token, or GitHub token. Central provisioning state is mirrored into protected `remote.json`; the Agent remains a separately installed privileged helper. |
